...
export function ProyectoPage({ proyecto, onBack }) {
  const { resumen, transacciones, loading, error, refetch } = useProyectoDetalle(proyecto.id)
  const [modalTipo, setModalTipo] = useState(null) // 'INGRESO' | 'GASTO' | null
  ...
  const recuperado = inversion > 0
    ? Math.min(100, Math.max(0, ((inversion - pendiente) / inversion) * 100))
    : 0
    ...
    <div className="min-h-screen bg-dark-bg text-accent-light">
      ...
      <header className="border-b border-border-dark/50 bg-dark-card sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center gap-3">
          <button
            onClick={onBack}
            className="text-accent-light/50 hover:text-accent transition-colors duration-200 text-xs sm:text-sm flex items-center gap-1 sm:gap-2 flex-shrink-0"
          >
            ← Proyectos
          </button>
          <div className="min-w-0">
            <h1 className="text-sm sm:text-lg font-bold text-accent-light truncate">{proyecto.nombre}</h1>
            {proyecto.descripcion && (
              <p className="text-accent-light/40 text-xs truncate hidden sm:block">{proyecto.descripcion}</p>
            )}
          </div>
        </div>
      </header>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className={`bg-dark-card border rounded-2xl p-4 sm:p-6 ${estaRecuperado ? 'border-accent/50' : 'border-border-dark'}`}>
                <p className="text-accent-light/50 text-xs uppercase tracking-widest mb-2 sm:mb-3">
                ...
                {estaRecuperado ? (
                  ...
                  <>
                    ...
                    <p className="text-accent-light/40 text-xs mt-1">
                      de {fmt(inversion, moneda)} inicial
                    </p>
                  </>
                )}
              </div>
                  ...
                  {transacciones.map((tx) => (
                    <li key={tx.id} className="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                        <span className={`text-base sm:text-xl flex-shrink-0 ${tx.tipo === 'INGRESO' ? 'text-accent' : 'text-danger'}`}>
                          {tx.tipo === 'INGRESO' ? '↑' : '↓'}
                        </span>
                        <div className="min-w-0">
